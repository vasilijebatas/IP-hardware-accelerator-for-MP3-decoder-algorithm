...
void mp3::init_frame_params(unsigned char *buffer)
{
	...
	for (int gr = 0; gr < 2; gr++) {
		...
		for (int ch = 0; ch < channels; ch++) {
			...
			// imdct(ch, gr, block_type, samples, prev_samples);
			imdct(gr, ch);
...
void mp3::stream(std::vector<unsigned char> &buffer, unsigned offset)
{
	unsigned sampling_rate = get_sampling_rate();
		unsigned channels = get_channel_mode() == mp3::Mono ? 1 : 2;
		snd_pcm_t *handle;
		...
		/* Start decoding. */
		while (is_valid() && buffer.size() > offset + get_header_size()) {
			init_header_params(&buffer[offset]);
			if (is_valid()) {
				init_frame_params(&buffer[offset]);
