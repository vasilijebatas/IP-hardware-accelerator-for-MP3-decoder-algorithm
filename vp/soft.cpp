...
Soft::Soft(sc_module_name name, char** argv) : sc_module(name), tsoc("tsoc")
{
	tsoc.register_b_transport(this, &Soft::b_transport);

	SC_THREAD(software);
	input = argv;

	buf = get_file(input[1]);
	offset = 0;
	tags = get_id3_tags(buf, offset);
	buffer = &buf[offset];
...
void Soft::b_transport(pl_t& pl, sc_time& offset)
{
	...
	default:
		...
		SC_REPORT_ERROR("SOFT", "TLM bad command");
		break;
	}
...
void Soft::software()
{
	...
	unsigned sampling_rate = get_sampling_rate();
	unsigned channels = get_channel_mode() == Soft::Mono ? 1 : 2;
	snd_pcm_t *handle;
 ...
 // Start decoding.
	while (is_valid() && buf.size() > offset + get_header_size()) {
		init_header_params(&buf[offset]);
		if (is_valid()) 
		{
				
				// init_frame_params(&buf[offset]);
			...
			for (int gr = 0; gr < 2; gr++) {
				...
				for (int ch = 0; ch < channels; ch++) {
					...
					pl.set_address(VP_ADDR_MEM_GR);
					pl.set_command(TLM_WRITE_COMMAND);
					pl.set_data_length(2);
					pl.set_data_ptr((unsigned char*)&gr);
					SC_REPORT_INFO("Soft", "Soft writes gr in memory");
					isoc_s->b_transport(pl, loct);
...
=======
					loct += sc_time(DELAY, SC_NS);
>>>>>>> Milos

					pl.set_address(VP_ADDR_MEM_CH);
					pl.set_command(TLM_WRITE_COMMAND);
					pl.set_data_length(2);
					pl.set_data_ptr((unsigned char*)&ch);
					SC_REPORT_INFO("Soft", "Soft writes ch in memory");
					isoc_s->b_transport(pl, loct);
					qk.set_and_sync(loct);
...
>>>>>>> Milos
					...
					for (int i = 0; i < 2; i++) {
					    for (int j = 0; j < 2; j++) {
					        int val = block_type[i][j];
					        unsigned char* ptr = (unsigned char*) &val;
					        val_block_type[i*2+j] = *ptr;
					    }
					}
					pl.set_address(VP_ADDR_MEM_BLOCK);
					pl.set_command(TLM_WRITE_COMMAND);
					pl.set_data_length(2*2);
					pl.set_data_ptr(val_block_type);
					SC_REPORT_INFO("Soft", "Soft writes block_type in memory");
					isoc_s->b_transport(pl, loct);
					qk.set_and_sync(loct);
...
>>>>>>> Milos
					...
					//3d matrix to array
					memcpy(val_samples, samples, 2*2*576*sizeof(sc_fixed<32,16>));

					pl.set_address(VP_ADDR_MEM_SAMPLES);
					pl.set_command(TLM_WRITE_COMMAND);
					pl.set_data_length(2*2*576*sizeof(sc_fixed<32,16>));
					pl.set_data_ptr(val_samples);
					SC_REPORT_INFO("Soft", "Soft writes samples in memory");
					isoc_s->b_transport(pl, loct);
					qk.set_and_sync(loct);
...
>>>>>>> Milos
					...
					start = 1;
					pl.set_address(VP_ADDR_HARD_START);
					pl.set_command(TLM_WRITE_COMMAND);
					pl.set_data_length(1);
					pl.set_data_ptr((unsigned char*)&start);
					SC_REPORT_INFO("Soft", "Soft sends flag to hard");
					isoc_s->b_transport(pl, loct);
					qk.set_and_sync(loct);
					...
					while(1)
					{
						if (ready == 1)
						{		
							SC_REPORT_INFO("Soft", "Soft received flag");
							ready = 0;
							break;
						}else
						{
							qk.set_and_sync(qk.get_local_time() + sc_time(20, SC_NS));
							loct = qk.get_local_time();
							SC_REPORT_INFO("Soft", string("Synced @ " + sc_time_stamp().to_string()).c_str());
						}
					}

					pl.set_address(VP_ADDR_HARD_OUT);
					pl.set_command(TLM_READ_COMMAND);
					pl.set_data_length(2*2*576*sizeof(sc_fixed<32,16>));
					pl.set_data_ptr(out);
					SC_REPORT_INFO("Soft", "Soft read output from hardware");
					isoc_s->b_transport(pl, loct);
					qk.set_and_sync(loct);
...
>>>>>>> Milos
					...
					for (int c = 0; c < 2; c++) {
				        for (int s = 0; s < 2; s++) {
				            for (int f = 0; f < 576; f++) {
				                memcpy(&samples[c][s][f], &out[(c * 2 * 576 + s * 576 + f) * sizeof(sc_fixed<32,16>)], sizeof(sc_fixed<32,16>));
				            }
				        }
				    }
		...
		int e = snd_pcm_writei(handle, get_samples(), 1152);
		if (e == -EPIPE)
...
void Soft::msg(const pl_t& pl)
{
	...
	SC_REPORT_INFO("SOFT", msg.c_str());
	trcnt++;
	SC_REPORT_INFO("SOFT", ("------------" + to_string(trcnt)).c_str());
}

vector<unsigned char> Soft::get_file(const char *dir)
{
...
vector<id3> Soft::get_id3_tags(std::vector<unsigned char> &buffer, unsigned &offset)
{
	std::vector<id3> tags;
	int i = 0;
	bool valid = true;
...
void Soft::init_header_params(unsigned char *buffer)
{
	if (buffer[0] == 0xFF && buffer[1] >= 0xE0) {
		this->buffer = buffer;
...
float Soft::get_mpeg_version()
{
	return mpeg_version;
}

/** Determine layer. */
void Soft::set_layer(unsigned char byte)
{
	byte = byte << 5;
	byte = byte >> 6;
	layer = 4 - byte;
 ...
 */
void Soft::set_bit_rate(unsigned char *buffer)
{
	if (mpeg_version == 1) {
		if (layer == 1) {
...
unsigned Soft::get_bit_rate()
{
	return bit_rate;
}

/** Sampling rate. */
void Soft::set_sampling_rate()
{
...
unsigned Soft::get_sampling_rate()
{
...
void Soft::set_padding()
{
	padding = buffer[2] & 0x02;
}

bool Soft::get_padding()
{
	return padding;
}
 ...
 * 3 -> Single channel
 */
void Soft::set_channel_mode(unsigned char *buffer)
{
...
Soft::ChannelMode Soft::get_channel_mode()
{
	return channel_mode;
}

/** Applies only to joint stereo. */
void Soft::set_mode_extension(unsigned char *buffer)
{
	if (layer == 3) {
...
unsigned Soft::get_frame_size()
{
	return frame_size;
}

unsigned Soft::get_header_size()
{
	return 4;
}
 ...
 */
void Soft::set_side_info(unsigned char *buffer)
{
	int count = 0;
 ...
 */
void Soft::set_main_data(unsigned char *buffer)
{
 ...
 * @param ch
 */
void Soft::unpack_scalefac(unsigned char *main_data, int gr, int ch, int &bit)
{
	int sfb = 0;
	int window = 0;
 ...
 * @param ch
 */
void Soft::unpack_samples(unsigned char *main_data, int gr, int ch, int bit, int max_bit)
{
	int sample = 0;
	int table_num;
 ...
 * @param ch
 */
void Soft::requantize(int gr, int ch)
{
	float exp1, exp2;
	int window = 0;
	int sfb = 0;
 ...
 * @param ch
 */
void Soft::reorder(int gr, int ch)
{
	int total = 0;
	int start = 0;
	int block = 0;
...
//			samples[start + block + 0] = this->samples[gr][ch][total + ss + sb_width * 0].to_float();
//			samples[start + block + 6] = this->samples[gr][ch][total + ss + sb_width * 1].to_float();
//			samples[start + block + 12] = this->samples[gr][ch][total + ss + sb_width * 2].to_float();
 ...
 * @param ch
 */
void Soft::alias_reduction(int gr, int ch)
{
...
/*
void Soft::imdct(int gr, int ch)
{
	static bool init = true;
	static sc_fixed<16,2> sine_block[4][36];
...
//		/* Overlap. 
		for (int i = 0; i < 18; i++) {
			samples[gr][ch][sample + i] = sample_block[i].to_float() + prev_samples[ch][block][i];
			prev_samples[ch][block][i] = sample_block[18 + i].to_float();
		}
		sample += 18;
	}
}*/
 ...
 * @param ch
 */
void Soft::frequency_inversion(int gr, int ch)
{
 ...
 * @param ch
 */
void Soft::synth_filterbank(int gr, int ch)
{
	...
	memcpy(samples2[gr][ch], pcm, 576 * 4);
...
void Soft::interleave()
{
	int i = 0;
	for (int gr = 0; gr < 2; gr++)
		for (int sample = 0; sample < 576; sample++)
			for (int ch = 0; ch < channels; ch++)
				pcm[i++] = samples[gr][ch][sample].to_float();
//				pcm[i++] = samples[gr][ch][sample];

}

float *Soft::get_samples()
{
	return pcm;
}
